...
## Execution Proof Chain（执行证明链）

Runtime 1.0 的标准是 **World Safety**：系统可证明自己不会二次改变世界。下述链条任一环节断掉，runtime 不成立。

```
Event Log
   ↓
ReplayContext (CompletedToolInvocations, StateChangesByStep)
   ↓
Runner 注入 context → step.Run(tool node)
   ↓
Adapter: InvocationLedger.Acquire
   ↓
ReturnRecordedResult → runConfirmation → 注入结果并 return（不调用 tool）
AllowExecute         → runNodeExecute  → 执行 tool → Commit（唯一执行 tool 的路径）
```

**唯一执行 tool 的代码路径**：仅当 Ledger 返回 `AllowExecute` 时，Adapter 进入 `runNodeExecute`，内部才调用 `Tools.Execute`。  
实现位置：[internal/agent/runtime/executor/node_adapter.go](internal/agent/runtime/executor/node_adapter.go) 第 161–196 行（`Acquire` 分支）；第 253 行起 `runNodeExecute`。  
时序图见 [design/execution-proof-sequence.md](design/execution-proof-sequence.md)。
...
- [internal/agent/runtime/executor/node_adapter.go](internal/agent/runtime/executor/node_adapter.go)：当 `InvocationLedger != nil` 时，先 `Acquire`；若 `ReturnRecordedResult` 则 Confirmation + 注入并 return；若 `AllowExecute` 则执行 tool 后 `Commit`。**1.0 要求**：持久化 job（`jobID != ""`）必须配置 Ledger，所有 tool 调用经 Ledger 裁决。无 Ledger 时的分支仅为**单进程/兼容路径**，不提供跨 worker 与 replay 的 World Safety 保证。
...
**Ledger 状态机（与 Store 映射）**：语义状态与持久化字段对应关系如下，不改变现有 schema。

| 语义状态 | 含义 | Store 对应 |
|----------|------|------------|
| NEW | 尚无记录，可发放执行许可 | 无记录（GetByJobAndIdempotencyKey 返回 nil） |
| INFLIGHT | 已占位、未提交，其他 worker 应等待 | Status=started，Committed=false |
| COMMITTED | 已提交成功，replay 仅恢复结果 | Status=success/confirmed，Committed=true，Result 非空 |
| RECOVERABLE | 可从事件流恢复（不依赖 Store） | 事件流 tool_invocation_finished(success) → CompletedToolInvocations[idempotency_key] |

Acquire 决策：NEW → AllowExecute；INFLIGHT → WaitOtherWorker；COMMITTED 或 replayResult → ReturnRecordedResult。
...
**1.0 证明**：下述四类场景的测试通过，则任意 agent step 在崩溃、网络中断、重启、双 worker 抢同一 step、replay 恢复下均**不会**重复触发外部副作用。测试见 [internal/agent/runtime/executor/ledger_1_0_test.go](internal/agent/runtime/executor/ledger_1_0_test.go)：(1) worker 在 tool 执行前崩溃 → TestLedger_1；(2) tool 执行后、commit 前崩溃 → TestLedger_2；(3) 两 worker 同时抢同一 step → TestLedger_3；(4) replay 恢复输出 → TestLedger_5、TestAdapter_Replay_InjectsResult_NoToolCall。
