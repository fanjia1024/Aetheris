...
type ExecutionNode struct {
	...
	DecisionSnapshot json.RawMessage  `json:"decision_snapshot,omitempty"` // plan 节点：Planner 决策快照（goal、task_graph_summary），供可追责
...
func BuildExecutionTree(events []jobstore.JobEvent) *ExecutionNode {
	...
	for i, e := range events {
		...
		case jobstore.DecisionSnapshot:
			if plan := byID["plan"]; plan != nil && len(e.Payload) > 0 {
				plan.DecisionSnapshot = e.Payload
			}
